...
class DB():
    ...
    def insert_players(self, player_id: int, player_name: str, player_pos: str) -> None:
        query = """
    ...
    def insert_teams(self, team_id: int, team_name: str) -> None:
        query = """
            INSERT OR IGNORE INTO TEAMS (Team_ID, Team_Name)
            VALUES (?, ?)
        """ 
        self.cursor.execute(query, (team_id, team_name))
        self.conn.commit()
    ...
    def insert_game(self, year: int, week: int, team_name: str) -> None:
        ...
        if not self.cursor.fetchone()[0] > 0:
            ...
            self.cursor.execute(query, (game_id + 1, year, week, team_name))
            self.conn.commit()
